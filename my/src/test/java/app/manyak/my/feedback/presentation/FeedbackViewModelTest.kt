...
class FeedbackViewModelTest {
    ...
    fun `이메일 형식이 어긋나면 보내지 않고 그 칸에 오류를 세운다`() =
        runTest {
            ...
            assertEquals(MyR.string.feedback_error_email, viewModel.uiState.value.emailErrorRes)
    ...
    fun `본문이 비고 이메일도 어긋나면 두 오류를 함께 세운다`() =
        runTest {
            ...
            assertEquals(0, repository.submitCount)
            assertEquals(MyR.string.feedback_error_empty, viewModel.uiState.value.bodyErrorRes)
            assertEquals(MyR.string.feedback_error_email, viewModel.uiState.value.emailErrorRes)
        }
