...
import app.manyak.analytics.entity.InviteCodeSource
...
internal fun DomainError.inviteCodeMessageRes(): Int {
    if (this !is DomainError.Server) return MyR.string.invite_code_error_failed
    return when {
        status == HTTP_BAD_REQUEST || status == HTTP_NOT_FOUND -> MyR.string.invite_code_error_not_found
        status == HTTP_CONFLICT && code == CODE_SELF -> MyR.string.invite_code_error_self
        status == HTTP_CONFLICT && code == CODE_ALREADY_REDEEMED -> MyR.string.invite_code_error_already_redeemed
        else -> MyR.string.invite_code_error_failed
    }
}
...
internal fun DomainError.inviteCodeErrorType(): String =
    when (inviteCodeMessageRes()) {
        MyR.string.invite_code_error_not_found -> "not_found"
        MyR.string.invite_code_error_self -> "self_code"
        MyR.string.invite_code_error_already_redeemed -> "already_redeemed"
