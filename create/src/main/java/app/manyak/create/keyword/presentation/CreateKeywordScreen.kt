...
private fun CreateKeywordContent(
    ...
    FocusScrollMargin {
        Column(
            ...
            CreateStepIndicator(
                currentStep = 0,
                stepNameRes = CreateR.string.create_step_keyword,
            )
...
private fun KeywordStepTitle(modifier: Modifier = Modifier) {
    Column(
        ...
        Text(
            text = stringResource(CreateR.string.create_keyword_title),
            style = ManyakTheme.typography.titleLarge,
            color = ManyakTheme.colors.text,
        )
        Text(
            text = stringResource(CreateR.string.create_keyword_description),
...
private fun CreateKeywordFooter(
    ...
    val footerErrorRes =
        when {
            ...
            state.validationErrorCategory == state.activeCategory -> CreateR.string.create_error_select_keyword
            state.showDuplicateNameFooterError -> CreateR.string.create_error_duplicate_name_footer
            else -> null
        }
    ...
    Row(horizontalArrangement = Arrangement.spacedBy(ManyakTheme.spacing.compact)) {
        if (!isFirstCategory) {
            FunnelNeutralButton(
                ...
                label = stringResource(CreateR.string.create_cta_previous),
                enabled = enabled,
                onClick = { onIntent(CreateKeywordIntent.GoPrevious) },
            )
        }
        val primaryLabelRes =
            if (isLastCategory) CreateR.string.create_cta_generate_storylines else CreateR.string.create_cta_next
        FunnelPrimaryButton(
            modifier = Modifier.weight(1f),
