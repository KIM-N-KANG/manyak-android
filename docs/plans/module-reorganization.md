...
- 공통 설계 정본: [하네스 Android 모듈 재구성 설계](../../../knk-harness/docs/design/1-2-android-design.md#1-2-2-모듈과-소유권)
- 현행 정본: [Android 설계](../../../knk-harness/docs/design/1-2-android-design.md), 로컬 검증 정책: [_project.md](./_project.md)
...
아래 파일 묶음은 기준 커밋의 이동 단위이며 링크는 이전된 실제 파일을 가리킵니다. 최종 포트와 검증 결과는 실행 기록에서 확인합니다. 최초 이전은 두 레포의 당시 브랜치에서 모듈당 로컬 커밋 하나로 진행했습니다. 후속 사용자 요청으로 하네스에 같은 티켓의 문서 브랜치를 만들고 기존 문서 커밋을 옮겼습니다. 새 티켓·PR·푸시는 이 작업의 범위에 포함하지 않습니다.
...
가장 큰 위험과 보완된 설계는 [Android ADR A-038](../../../knk-harness/docs/adr/1-3-android-adr.md#a-038)가 소유합니다. 특히 파일 경로를 먼저 일괄 바꾸고 나중에 순환을 해결하는 방식은 피하고, 계약·구현·provider·소비자·테스트를 이동 단위로 묶습니다.
...
- auth 인증·세션 모듈 분리.
- 토큰·계정 연동·가입 안내 및 마이·저장소 테스트, 앱 Hilt 조립, ktlint·detekt 통과; 저장 형식과 로그인 순서 보존

### report

- 공유 신고 업무 report 모듈 분리.
- 채팅·스튜디오·상세 및 홈 테스트, 앱 Hilt 조립, ktlint·detekt 통과; 네 신고 화면의 상태·스코프와 리소스 값 보존

### home

- home 화면·데이터 모듈 분리.
- 홈 테스트·공유 DTO 소비자 컴파일·앱 Hilt 조립·ktlint·detekt 통과; 기존 공용 요약 DTO 재사용과 리소스 소유권 정리

### chat

- chat 목록·채팅방 및 데이터 모듈 분리.
- 채팅·SSE·설정과 제작·상세 테스트, 앱 Hilt 조립, ktlint·detekt 통과; 채팅 생성 포트만 공통 공개 및 기존 상태·저장 키 보존
...
- my 세부 기능·프로필·이프·초대 데이터 모듈 분리.
- 프로필·이프·초대·문의·탈퇴·라이선스를 내부 패키지로 나눴습니다. my·인증 테스트, 기존 프로필 JSON 및 공유 DataStore 키·정리 호환 테스트, 변경 모듈 정적 검사와 app Hilt 컴파일을 통과했습니다.
...
| `0540d3b1` | app 최종 조립·잔여 core 제거·경계 검사 |

## 문서 후속 정리

- 하네스 `docs/KNK-1197-improve-folder-structure` 브랜치에 기존 문서 커밋 `4c3468d`·`c2abefc`를 보존했습니다. 로컬 `dev`는 해당 두 커밋 이전의 `eaf081c`로 복원했습니다.
- 대응 브랜치와 전환 이력은 계획·스펙 문서에 기록합니다. `CLAUDE.md`에는 현재 구조와 지속적으로 적용할 작업 규칙만 둡니다. 기존 기능 계획의 과거 모듈 경로·검증 명령은 이력으로 남기고 현재 구조 안내를 붙였습니다.
- 등록에서 빠진 뒤에도 작업 트리에 남아 있던 `core/`·`feature/` 디렉터리를 삭제했습니다. 추적 파일 0개에 옛 `build/` 산출물과 빈 `src` 껍데기뿐이었고, 남겨 두면 검색이 옛 산출물을 잡고 폴더만 보면 이전 구조가 유효한 것처럼 보입니다.
- `CLAUDE.md` 정본 표에 폴더 트리를 가진 `README.md`와 이 문서를 추가하고, 변경 모듈 단위 검증 명령(`:<모듈>:ktlintCheck :<모듈>:detekt :<모듈>:testDebugUnitTest`)을 명시했습니다.
- 이 후속 정리는 문서와 Git 브랜치만 변경합니다. 문서 링크·diff와 커밋 보존을 검증하며, 앞서 통과한 앱 빌드·테스트를 반복하지 않습니다.
