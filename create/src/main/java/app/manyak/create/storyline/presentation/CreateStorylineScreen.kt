...
import app.manyak.create.presentation.component.FunnelNeutralButton
...
import app.manyak.create.presentation.component.StorylineGeneratingContent
            ...
            viewModel.uiEffect.collect { effect ->
                when (effect) {
                    ...
                    CreateStorylineEffect.ShowRatingSyncFailed ->
                        Toast
                            .makeText(context, CreateR.string.create_storyline_rating_sync_failed, Toast.LENGTH_SHORT)
                            .show()
...
private fun CreateStorylineContent(
    ...
    Column(
        ...
        CreateStepIndicator(
            currentStep = 1,
            stepNameRes = CreateR.string.create_step_storyline,
        )
        when (state.content) {
    ...
    LazyColumn(modifier = modifier, state = listState) {
        ...
        when {
                        ...
                        text =
                            stringResource(
                                if (state.storylines.isEmpty()) {
                                    CreateR.string.create_storyline_error_generate
                                } else {
                                    CreateR.string.create_storyline_error_regenerate
                                },
                            ),
                        isError = true,
                    )
            ...
            state.storylines.isEmpty() ->
                item { StorylineNotice(text = stringResource(CreateR.string.create_storyline_empty), isError = false) }
        }
...
private fun StorylineStepTitle(modifier: Modifier = Modifier) {
    Column(
        ...
        Text(
            text = stringResource(CreateR.string.create_storyline_title),
            style = ManyakTheme.typography.titleLarge,
            color = ManyakTheme.colors.text,
        )
        Text(
            text = stringResource(CreateR.string.create_storyline_description),
...
private fun StorylineTabs(
    ...
    val labels = stringArrayResource(CreateR.array.create_storyline_tab_labels)
    ...
    Column(
        ...
        Row(
            ...
            FunnelNeutralButton(
                ...
                label = stringResource(CreateR.string.create_cta_regenerate),
                enabled = true,
                onClick = { onIntent(CreateStorylineIntent.Regenerate) },
            )
            FunnelPrimaryButton(
                modifier = Modifier.weight(1f),
                label = stringResource(CreateR.string.create_cta_select_storyline),
