package app.manyak.my.feedback.presentation
    ...
    LaunchedEffect(viewModel, lifecycleOwner) {
        lifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
            viewModel.uiEffect.collect { effect ->
                val message =
                    when (effect) {
                        FeedbackEffect.Submitted -> MyR.string.feedback_submitted
                        FeedbackEffect.SubmitFailed -> MyR.string.feedback_submit_failed
                    }
...
private fun FeedbackHeadline(modifier: Modifier = Modifier) {
    Column(
        ...
        Text(
            text = stringResource(MyR.string.feedback_title),
            style = ManyakTheme.typography.titleLarge,
            color = ManyakTheme.colors.text,
        )
        Text(
            text = stringResource(MyR.string.feedback_description),
...
private fun FeedbackBodyField(
    ...
    Column(
        ...
        MyFieldLabel(text = stringResource(MyR.string.feedback_body_label), isRequired = true)
        ManyakMultilineTextField(
            modifier = Modifier.fillMaxWidth(),
            value = state.body,
            onValueChange = { onIntent(FeedbackIntent.BodyChanged(it)) },
            placeholder = stringResource(MyR.string.feedback_body_placeholder),
...
private fun FeedbackEmailField(
    ...
    Column(
        ...
        MyFieldLabel(text = stringResource(MyR.string.feedback_email_label))
        ManyakTextField(
            modifier = Modifier.fillMaxWidth(),
            value = state.email,
            onValueChange = { onIntent(FeedbackIntent.EmailChanged(it)) },
            placeholder = stringResource(MyR.string.feedback_email_placeholder),
            enabled = !state.isSubmitting,
        ...
        MyFieldMessage(
            text = stringResource(state.emailErrorRes ?: MyR.string.feedback_email_description),
            isError = state.emailErrorRes != null,
        )
    }
}
...
private fun FeedbackSubmit(
    ...
    Column(
        ...
        MyPrimaryButton(
            modifier = Modifier.fillMaxWidth(),
            label = stringResource(MyR.string.feedback_submit),
