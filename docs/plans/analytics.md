...
- 작성일: 2026-09-04
- 근거 정본: 하네스 `spec/6-analytics.md §6-4-2`(카탈로그)·`§6-4-2-15`(앱 보강)·`§6-4-2-16`(플랫폼 적용 범위), `design/1-2-android-design.md §1-2-8`(화면별 발화 지점)
...
앱 보강 이벤트 중 웹에 같은 화면이 있는 항목은 하네스 `spec/6-analytics.md §6-4-2-16` 의 표가 소유한다. 이 레포는
그 표를 다시 적지 않는다.
