...
7. 일반 입력 모드 컴포저의 위 여백(`spacing.compact` 8dp)을 없애 블럭 모드와 같게 목록 바로 아래에서 시작합니다. 웹은 두 모드 모두 위 8px을 두지만 앱은 두지 않기로 했습니다.
8. 하네스 Android spec의 "채팅 헤더는 고정" 문구와 design을 현행화했습니다.
...
- 확인 중 추천 선택지를 잘못 눌러 dev 채팅방에 턴이 하나 전송됐습니다(무료 체험 적용가 0 이프).
