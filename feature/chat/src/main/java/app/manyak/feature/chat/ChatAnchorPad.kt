...
internal fun AnchorStreamingTurn(
    ...
    val anchored = remember { AnchorFlag() }
    ...
    SideEffect {
        ...
        if (anchored.value) return@SideEffect
        anchored.value = true
        val viewport = listState.layoutInfo.viewportHeight()
        if (viewport > 0) padPx.intValue = viewport
    ...
    LaunchedEffect(state.isStreaming) {
        ...
        var anchoredViewport = listState.layoutInfo.viewportHeight()
        snapshotFlow { listState.layoutInfo }.collect { info ->
            val viewport = info.viewportHeight()
            if (viewport <= 0) return@collect
            if (viewport != anchoredViewport) {
...
/** 스트림 하나에 첫 앵커를 한 번만 걸기 위한 표식. 스냅샷 상태가 아니라 재구성을 만들지 않는다. */
private class AnchorFlag {
