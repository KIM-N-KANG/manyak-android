...
internal fun StorylineGeneratingContent(modifier: Modifier = Modifier) {
    GeneratingLoadingContent(
        modifier = modifier,
        titleRes = CreateR.string.create_storyline_loading_title,
        descriptionRes = CreateR.string.create_storyline_loading_description,
        loadingLabelRes = CreateR.string.create_storyline_loading_label,
        phrasesRes = CreateR.array.create_storyline_loading_phrases,
        hints =
            listOf(
                GenerationHint(delayMs = 15_000, textRes = CreateR.string.create_storyline_loading_hint_delayed),
                GenerationHint(delayMs = 30_000, textRes = CreateR.string.create_storyline_loading_hint_long),
            ),
    )
}
...
private fun ElapsedSeparator(
    ...
    Row(
        ...
        Text(
            text = stringResource(CreateR.string.create_loading_hint_elapsed, seconds),
            style = ManyakTheme.typography.bodyMedium,
            color = ManyakTheme.colors.textSubtlest,
        )
        SeparatorLine(modifier = Modifier.weight(1f))
