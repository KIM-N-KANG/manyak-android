...
interface StoryCompletionRequestDao {
    @Query("SELECT * FROM story_completion_request ORDER BY submittedAt DESC")
    fun observeAll(): Flow<List<StoryCompletionRequestEntity>>

    @Query("SELECT * FROM story_completion_request ORDER BY submittedAt DESC")
    suspend fun findAll(): List<StoryCompletionRequestEntity>
     ...
     */
    @Query("DELETE FROM pending_story_creation WHERE id = :draftId AND generation = :generation")
    ...
    suspend fun submit(entity: StoryCompletionRequestEntity) {
        releaseDraft(PendingStoryCreationEntity.SINGLE_ROW_ID, entity.generation)
        upsert(entity)
    }
    ...
    @Query("DELETE FROM story_completion_request")
    suspend fun clear()
}
